    ...
    public abstract class BaseRepository
    {
        ...
        public virtual IORMRepository<TEntity> For<TEntity>() where TEntity : BaseEntity, new()
        {
            ...
            lock (_lockdic)
            {
                ...
                {
                    ORMRepository<TEntity> rrmRe = new ORMRepository<TEntity>(this.connectionString);
        ...
        protected PagedList<T> PageGet<T>(PageView view, string sql,object param) where T :class,new()
        {
            ...
            if (view.Total == 0)
                pList.Total = this.GetScalar<int>(EntityMSSQLBuilder.BuildPageCountSql(sql), param);
            else
            ...
            if (pList.Total > 0)
            {
               ...
               // }
                ...
                var list = this.Query<T>(EntityMSSQLBuilder.BuildPageSql(sql, view), param);
                pList.DataList = list;
            }
            else
        ...
        {
            return DBHelper.ExecuteCommand(this.connectionString, safeSql, param,cmdType);
        }
        ...
        {
            object r = DBHelper.GetScalar(this.connectionString, safeSql, param,cmdType);            
            return r == DBNull.Value ? default(T) : (T)r;
        }
        ...
        {
            return DBHelper.GetReader(this.connectionString, safeSql, param,cmdType);
        }
        ...
        {
            return DBHelper.GetDataSet(this.connectionString,safeSql, param,cmdType);
        }
        ...
        {
            bool isCustomRow = false;
            if(typeof(TResult) == typeof(CustomRow))
            {
                isCustomRow = true;
            }

            using (IDataReader reader = DBHelper.GetReader(connectionString, safeSql, param,cmdType))
            {
                if (!isCustomRow)
                {
                    return ConvertHelper.ConvertToList<TResult>(reader);
                }
                return CustomRow.Fill(reader) as List<TResult>;
        ...
        {
            return Query<TResult>(safeSql,null,cmdType)[0];
        }
        protected TResult Get<TResult>(string safeSql, object param, CommandType cmdType = CommandType.Text) where TResult : class,new()
        {
            ...
            using (IDataReader reader = DBHelper.GetReader(connectionString, safeSql, param, cmdType))
            {
                return ConvertHelper.ConvertToList<TResult>(reader).FirstOrDefault();
            }
        }
