    ...
    public class ORMOracleRepository<TEntity> : IORMRepository<TEntity> where TEntity : BaseEntity, new()
     {
        ...
        public ORMOracleRepository(string connectionString)
        {
         ...
         public int Delete(Expression<Predicate<TEntity>> where) 
         {
             ...
             return OracleDBHelper .ExecuteCommand(connectionString, sql, param);
         }
         #endregion
         ...
         {             
             return EntityORASQLBuilder.BuildUpdateSql(metadeta, entity.PropertyChangedList,HasWhere);
         }
         ...
         public int Update(TEntity entity, Expression<Predicate<TEntity>> where) 
         {
            ...
            int result= OracleDBHelper.ExecuteCommand(this.connectionString, sql,param);
         ...
         public long Add(TEntity entity) 
         {
             ...
             long id= OracleDBHelper.GetScalar<long>(connectionString, sql, entity);
             entity.RemoveUpdateColumn();//清除掉已更新的字段
             return id;
         }
         ...
         private List<TEntity> FindAll(Expression<Predicate<TEntity>> where, string showFields, EntityMeta meta) 
         {
             ...
             return OracleDBHelper.Query<TEntity>(connectionString, sql, param);
         }
         ...
         private PagedList<TEntity> GetPage(Expression<Predicate<TEntity>> where, string showFields, EntityMeta meta,PageView view)
         {
             ...
             if (view.Total == 0)
                 pList.Total = OracleDBHelper.GetScalar<int>(this.connectionString, EntityORASQLBuilder.BuildPageCountSql(sql), param);
             else
                 pList.Total = view.Total;

             if (pList.Total > 0)
             {
                 pList.DataList=OracleDBHelper.Query<TEntity>(this.connectionString, EntityORASQLBuilder.BuildPageSql(sql, view), param);
         ...
         protected TResult ExecOne<TResult>(Expression fields, Expression where, string execType)
         {
             ...
             return OracleDBHelper.GetScalar<TResult>(this.connectionString, sql, param);
