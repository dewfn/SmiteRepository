...
namespace SmiteRepository.Dapper
{
...
#pragma warning restore 612, 618
    ...
    sealed partial class CustomPropertyTypeMap : SqlMapper.ITypeMap
    {
        ...
        public CustomPropertyTypeMap(Type type)
        {
            if (type == null)
                throw new ArgumentNullException("type");
            _meta = EntityReflect.GetDefineInfoFromType(type);
        ...
        public ConstructorInfo FindConstructor(string[] names, Type[] types)
        {
            var constructors = _meta.EntityType.GetConstructors(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            foreach (ConstructorInfo ctor in constructors.OrderBy(c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1)).ThenBy(c => c.GetParameters().Length))
            {
                ParameterInfo[] ctorParameters = ctor.GetParameters();
                if (ctorParameters.Length == 0)
        ...
        public SqlMapper.IMemberMap GetConstructorParameter(ConstructorInfo constructor, string columnName)
        {
            //dewfn 修改过，适合自定义属性
            var parameters = constructor.GetParameters();
            EntityColumnMeta col = _meta.Columns.FirstOrDefault(x => string.Equals(x.ColumnName, columnName, StringComparison.OrdinalIgnoreCase));
            columnName = col.FieldName;

            return new SimpleMemberMap(columnName, parameters.FirstOrDefault(p => string.Equals(p.Name, columnName)));
        }
        ...
        public SqlMapper.IMemberMap GetMember(string columnName)
        {
         ...
         EntityColumnMeta col= _meta.Columns.FirstOrDefault(x => string.Equals(x.ColumnName , columnName,StringComparison.OrdinalIgnoreCase));
         columnName=col.FieldName;
            var property = _properties.FirstOrDefault(p=> string.Equals(p.Name,columnName) );
