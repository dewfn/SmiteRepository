...
namespace SmiteRepository.Mysql
{
    ...
    public class ORMMysqlRepository<TEntity> : IORMRepository<TEntity> where TEntity : BaseEntity, new()
    {

        internal ORMMysqlRepository(string connectionString, EntityMeta Meta)
         {
             this.connectionString = connectionString;
             meta = Meta;
         }
        public ORMMysqlRepository(string connectionString)
        {
        ...
        public int Delete(Expression<Predicate<TEntity>> where)
        {
            ...
            return MysqlDBHelper.ExecuteCommand(connectionString, sql, param);
        }
        #endregion
        ...
        private string GetUpdateChangeColumnsSql(bool HasWhere, TEntity entity, EntityMeta metadeta)
        {
            ...
            {
#endif
                return EntityMYSQLBuilder.BuildUpdateSql(metadeta, entity.PropertyChangedList, HasWhere);
        ...
        public int Update(TEntity entity, Expression<Predicate<TEntity>> where)
        {
            ...
            int result = MysqlDBHelper.ExecuteCommand(this.connectionString, sql, param);
        ...
        private string GetInsertChangeColumnsSql(TEntity entity, EntityMeta metadeta)
        {
            ...
            {
#endif
                return EntityMYSQLBuilder.BuildInsertSql(metadeta, entity.PropertyChangedList);
#if Safety
            }
#endif
        }
        ...
        public long Insert(TEntity entity)
        {
            ...
            long id = MysqlDBHelper.GetScalar<long>(connectionString, sql, entity);
            entity.RemoveUpdateColumn();//清除掉已更新的字段
            return id;
        }
        ...
        private TEntity Find(Expression<Predicate<TEntity>> where, string showFields, string orderFields)
        {
            ...
            return MysqlDBHelper.Query<TEntity>(connectionString, sql, param).FirstOrDefault();
        }
        ...
        private List<TEntity> FindAll(Expression<Predicate<TEntity>> where, string showFields, EntityMeta meta)
        {
            ...
            return MysqlDBHelper.Query<TEntity>(connectionString, sql, param);
        }
        ...
        private PagedList<TEntity> GetPage(Expression<Predicate<TEntity>> where, string showFields, EntityMeta meta, PageView view)
        {
            ...
            if (view.Total == 0)
                pList.Total = MysqlDBHelper.GetScalar<int>(this.connectionString, EntityMYSQLBuilder.BuildPageCountSql(sql), param);
            else
                pList.Total = view.Total;

            if (pList.Total > 0)
            {
                pList.DataList = MysqlDBHelper.Query<TEntity>(this.connectionString, EntityMYSQLBuilder.BuildPageSql(sql, view), param);
        ...
        protected TResult ExecOne<TResult>(Expression fields, Expression where, string execType)
        {
            ...
            return MysqlDBHelper.GetScalar<TResult>(this.connectionString, sql, param);
