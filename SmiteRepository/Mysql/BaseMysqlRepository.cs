    ...
    public abstract class BaseMysqlRepository
    {
        ...
        public virtual IORMRepository<TEntity> For<TEntity>() where TEntity : BaseEntity, new()
        {
            ...
            lock (_lockdic)
            {
                ...
                {
                    IORMRepository<TEntity> rrmRe = new ORMMysqlRepository<TEntity>(this.connectionString);
        ...
        protected PagedList<T> PageGet<T>(PageView view, string sql, object param) where T : class,new()
        {
            ...
            if (view.Total == 0)
                pList.Total = this.GetScalar<int>(EntityMYSQLBuilder.BuildPageCountSql(sql), param);
            else
            ...
            if (pList.Total > 0)
            {
                ...
                var list = this.Query<T>(EntityMYSQLBuilder.BuildPageSql(sql, view), param);
                pList.DataList = list;
            }
            else
        ...
        {
            return MysqlDBHelper.ExecuteCommand(this.connectionString, safeSql, param, cmdType);
        }
        ...
        {
            return MysqlDBHelper.GetScalar<T>(this.connectionString, safeSql, param, cmdType);
        ...
        {
            return MysqlDBHelper.GetReader(this.connectionString, safeSql, param, cmdType);
        }
        ...
        {
            return MysqlDBHelper.GetDataSet(this.connectionString, safeSql, param, cmdType);
        }
        ...
        {
            return MysqlDBHelper.Query<TResult>(connectionString, safeSql, param, cmdType);
        }
